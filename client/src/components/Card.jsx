                    ...
                    <h1 className="text-4xl font-bold block text-slate-300">{props.title}</h1>
                        {props.title === "To-Read" && (
                            <div className="flex items-center justify-center" >
                                <input 
                                    type="text" 
                                    className="mr-2 p-1 w-60 h-8 rounded bg-stone-200 text-black text-sm focus:outline-stone-800"
                                    value={url}
                                    onChange={(e) => setUrl(e.target.value)}
                                    placeholder = "Copy + Paste URL"
                                />
                                <button 
                                className="pr-3 pl-3 text-sm h-9 hover:bg-stone-800 bg-stone-900 text-slate-300 rounded"
